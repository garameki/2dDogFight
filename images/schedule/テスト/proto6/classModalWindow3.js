...
ModalWindow.prototype.appear = function(absoluteX,absoluteY){
	if(!ModalWindow.isExist){
		ModalWindow.isExist=true;


		//mwの全ての要素のzindexを更新

		this.eleBase.style.zIndex=NumZindex.up();
		var updateZindex=function(ele){
			ele.style.zIndex=NumZindex.up();
			for(var ii=0,len=ele.children.length;ii<len;ii++){
				updateZindex(ele.children[ii]);
			};
		};
		updateZindex(this.eleDiv);
...
//ここではeleSpanはbodyにアタッチされていないのでeleSpan.clientWidthは0
		...
		body.appendChild(this.eleBase);//先にappendしておくと、widthなどのpropertyの値が得られて便利
		body.appendChild(this.eleDiv);
...
var hoge=setInterval(function(){
	clearInterval(hoge);
	console.log("classModalWindowJS   this.eleDiv.id's parent =",mm.eleDiv.parentNode);
},100);
//		eleInp.focus();
...
console.log("classModalWindowJS vanish---this.eleBase.parentNode=",this.eleBase.parentNode," this.eleDiv.parentNode=",this.eleDiv.parentNode);
	//modal window消す

	if(this.eleBase.parentNode){

		this.eleBase.parentNode.removeChild(this.eleBase);
		this.eleDiv.parentNode.removeChild(this.eleDiv);
		ModalWindow.isExist=false;
...
Object.defineProperty(ModalWindow,'ans',{value:true,writable:true,enumerable:true,configurable:true});//モーダルウィンドウからの答え
Object.defineProperty(ModalWindow,'isExist',{value:false,writable:true,enumerable:true,configurable:true});//モーダルウィンドウからの答え。出てるときはtrue
